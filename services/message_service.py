...
class MessageService:
    ...
    async def send_interactive_post(
        ...
        try:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # MEJORA: Crear misiones descriptivas basadas en el contenido
            # ═══════════════════════════════════════════════════════════════════

            # Generar nombre descriptivo basado en el contenido del mensaje
            clean_text = re.sub(r'[^\w\s]', '', text[:100])  # Limpiar caracteres especiales
            words = clean_text.split()[:5]  # Primeras 5 palabras
            content_preview = ' '.join(words) if words else "Nueva publicación"

            # Emojis según canal
            mission_emoji = "💎" if channel_type == "vip" else "✨"

            # Nombre descriptivo con preview del contenido
            mission_name = f"{mission_emoji} {content_preview[:40]}..."

            # Descripción con extracto del mensaje
            text_preview = text[:150] if len(text) > 150 else text
            mission_description = (
                f"🎯 **Nueva publicación de Diana**\n\n"
                f"_{text_preview}_{'...' if len(text) > 150 else ''}\n\n"
                f"💫 Reacciona para mostrar tu aprecio y ganar besitos"
            )

            # Recompensa mayor para incentivar participación
            reward = 3 if channel_type == "vip" else 2
            ...
            await mission_service.create_mission(
                ...
                action_data={
                    "target_message_id": real_message_id,
