...
def get_shop_main_kb(catalog_data: Dict[str, Any]) -> InlineKeyboardMarkup:
    ...
    builder.button(text="📦 Mi Inventario", callback_data="shop_inventory")
    builder.button(text="📊 Mis Compras", callback_data="shop_purchase_history")
    ...
    # Organizar botones
    if len(categories) <= 2:
        builder.adjust(len(categories) if categories else 1, 2, 1, 1)
    else:
...
def get_shop_category_kb(category: str, items: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    ...
    # Paginación si hay más de 8 artículos
    if len(items) > 8:
        builder.button(
            text="➡️ Ver Más",
            callback_data=f"shop_category_page:{category}:1"
        )
    
    # Navegación
    builder.button(text="⬅️ Volver a Tienda", callback_data="shop_main")
    builder.button(text="🏠 Menú Principal", callback_data="menu_principal")
...
def get_shop_item_detail_kb(
    ...
    if can_purchase:
        ...
        builder.button(
            ...
            callback_data=f"shop_confirm:{item_id}"
        )
    else:
        builder.button(
            text="❌ No Disponible",
    ...
    builder.button(text="🛒 Tienda Principal", callback_data="shop_main")
    builder.button(text="🏠 Menú Principal", callback_data="menu_principal")
...
def get_shop_inventory_kb(inventory: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    ...
    usable_items = [item for item in inventory if not item["is_used"]][:5]
    ...
    # Navegación
    builder.button(text="🛒 Ir a Tienda", callback_data="shop_main")
    builder.button(text="🏠 Menú Principal", callback_data="menu_principal")
...
def get_shop_vip_preview_kb() -> InlineKeyboardMarkup:
    """Teclado para preview de artículos VIP."""
    builder = InlineKeyboardBuilder()
    
    builder.button(text="💎 Suscribirse a VIP", callback_data="vip_subscription")
    builder.button(text="❓ Beneficios VIP", callback_data="vip_benefits")
    builder.button(text="⬅️ Volver a Tienda", callback_data="shop_main")
...
def get_shop_error_kb() -> InlineKeyboardMarkup:
    ...
    builder.button(text="🔄 Reintentar", callback_data="shop_main")
    builder.button(text="🏠 Menú Principal", callback_data="menu_principal")
