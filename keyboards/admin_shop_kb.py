...
def get_admin_shop_items_kb(items: List[Any]) -> InlineKeyboardMarkup:
    ...
    if len(items) > 8:
        builder.button(text="➡️ Ver Más", callback_data="admin_shop_items_page:1")
...
def get_admin_shop_item_actions_kb(item_id: int, is_active: bool) -> InlineKeyboardMarkup:
    ...
    builder.button(text="📊 Ver Estadísticas", callback_data=f"admin_shop_item_stats:{item_id}")
...
def get_admin_shop_config_kb() -> InlineKeyboardMarkup:
    """Teclado para configuración de tienda."""
    builder = InlineKeyboardBuilder()
    
    # Configuraciones disponibles
    builder.button(text="💰 Configurar Descuentos VIP", callback_data="admin_shop_vip_discounts")
    builder.button(text="🎯 Configurar Niveles", callback_data="admin_shop_level_config")
    builder.button(text="🔗 Integración Narrativa", callback_data="admin_shop_narrative_config")
    builder.button(text="📧 Notificaciones", callback_data="admin_shop_notifications")
...
def get_admin_shop_discounts_kb() -> InlineKeyboardMarkup:
    """Teclado para gestión de descuentos."""
    builder = InlineKeyboardBuilder()
    
    builder.button(text="➕ Crear Descuento", callback_data="admin_shop_create_discount")
    builder.button(text="📋 Ver Descuentos Activos", callback_data="admin_shop_list_discounts")
    builder.button(text="💎 Descuentos VIP", callback_data="admin_shop_vip_discounts")
    builder.button(text="⬅️ Volver", callback_data="admin_shop_main")
