...
@pytest.mark.asyncio
async def test_handle_reaction_callback_success(mock_callback, mock_session, mock_bot):
    """Test successful reaction callback handling."""
    ...
    with patch('handlers.reaction_callback.validate_message', return_value=True):
        ...
        with patch('handlers.reaction_callback.MessageService') as mock_service_class:
            ...
            with patch('handlers.reaction_callback.ChannelService') as mock_channel_service_class:
                ...
                mock_channel_service.get_reaction_points.return_value = {"👍": 1.0}
                ...
                with patch('services.point_service.PointService') as mock_point_service_class:
                    ...
                    with patch('services.mission_service.MissionService') as mock_mission_service_class:
                        ...
                        # Mock BOT_MESSAGES
                        with patch('handlers.reaction_callback.BOT_MESSAGES', {
                            ...
                            mock_callback.answer.assert_called_once_with(
                                "¡Has ganado 1.0 puntos!"
                            )


@pytest.mark.asyncio
async def test_handle_reaction_callback_duplicate_reaction(mock_callback, mock_session, mock_bot):
    """Test handling of duplicate reaction (should return None from register_reaction)."""
    # Mock the validate_message function to return True
    with patch('handlers.reaction_callback.validate_message', return_value=True):
        # Mock the MessageService methods
        with patch('handlers.reaction_callback.MessageService') as mock_service_class:
            mock_service = AsyncMock()
            mock_service.register_reaction.return_value = None  # Return None for duplicate reaction
            mock_service_class.return_value = mock_service
            ...
            with patch('handlers.reaction_callback.BOT_MESSAGES', {
                ...
                # Verify that callback.answer was called with duplicate reaction message
                mock_callback.answer.assert_called_once_with(
                    "Ya has reaccionado a este post.",
                    show_alert=True
                )


@pytest.mark.asyncio
async def test_handle_reaction_callback_invalid_message(mock_callback, mock_session, mock_bot):
    """Test handling of invalid message (validate_message returns False)."""
    # Mock the validate_message function to return False
    with patch('handlers.reaction_callback.validate_message', return_value=False):
        # Mock logger
        with patch('handlers.reaction_callback.logger') as mock_logger:
            # Call the handler
            await handle_reaction_callback(mock_callback, mock_session, mock_bot)
            
            # Verify that callback.answer was called with no parameters
            mock_callback.answer.assert_called_once_with()
